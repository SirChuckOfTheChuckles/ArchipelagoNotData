...
from ..locations import MISSION_TO_VICTORY_LOCATION
...
class SC2MissionOrder:
    ...
    def get_completion_condition(self, player: int) -> Callable[[CollectionState], bool]:
        ...
        final_locations = [
            MISSION_TO_VICTORY_LOCATION[mission.mission].global_name()
            for mission in self.get_final_missions()
        ]
        def is_completed(state: CollectionState) -> bool:
            return all(state.can_reach_location(loc, player) for loc in final_locations)
        return is_completed
